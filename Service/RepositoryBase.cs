...
namespace Service
{
     ...
     public abstract class RepositoryBase<T> : IRepository<T> where T : class
     {
        ...
        {
            return await Task.Run(() => Context.Set<T>().AsNoTracking().SingleOrDefault(predicate));
        }
        ...
        {
            return await Task.Run(() => Context.Database.ExecuteSqlCommand(sql, para));
        }
        #endregion
        ...
        {
            return predicate != null ? Context.Set<T>().Where(predicate).AsNoTracking<T>() : Context.Set<T>().AsQueryable<T>().AsNoTracking<T>();
        }
        /// <summary>
        ...
        {
            return predicate != null ? await Task.Run(() => Context.Set<T>().Where(predicate).AsNoTracking<T>()) : await Task.Run(() => Context.Set<T>().AsQueryable<T>().AsNoTracking<T>());
        }
        ...
        {
            return predicate != null ? Context.Set<T>().Where(predicate).AsNoTracking().ToList() : Context.Set<T>().AsQueryable<T>().AsNoTracking().ToList();
        }
        // <summary>
        ...
        {
            return predicate != null ? await Task.Run(() => Context.Set<T>().Where(predicate).AsNoTracking().ToList()) : await Task.Run(() => Context.Set<T>().AsQueryable<T>().AsNoTracking().ToList());
        }
        ...
        {
            return predicate != null ? Context.Set<T>().Where(predicate) as DbQuery<T> : Context.Set<T>();
        }
        /// <summary>
        /// 获取DbQuery的列表（异步方式）
        /// </summary>
        ...
        {
            return predicate != null ? await Task.Run(() => Context.Set<T>().Where(predicate) as DbQuery<T>) : await Task.Run(() => Context.Set<T>());
        }
        ...
        {
            return await Task.Run(() => Context.Database.SqlQuery<T>(sql, para));
        }
        ...
        {
            return await Task.Run(() => Context.Database.SqlQueryForDynamic(sql, para));
        }
        ...
        {
            return await Task.Run(() => Context.Database.SqlQuery(typeof(T), sql, para).Cast<T>().ToList());
        }
        ...
        {
            return await Task.Run(() => Context.Database.SqlQuery<T1>(sql, para).ToList());
        }
        ...
        {
            var entry = Context.Set<T>().Where(predicate);
            return (entry.Any());
        }
        /// <summary>
        /// 验证当前条件是否存在相同项（异步方式）
        ...
        {
            return Context.Database.SqlFunctionForDynamic(procname, parameter);
        }
        #endregion
